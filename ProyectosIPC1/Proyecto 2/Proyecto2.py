...
class Book:
    ...
    def Search_Book_Filter(self,Titulo,De,A,Autor):
        Total = []
        if De and A == "0" and Autor == " " and Titulo == " ":
            ## Ninguno
            for i in range(len(self.Data)):
                search = {
                "ISBN": self.Data[i][0],
        ...
        if De and A != "0" and Autor != " " and Titulo == " ":
            for i in range(len(self.Data)):
                if int(De)<=self.Data[i][4]<=int(A) and self.Data[i][1] == Autor:
                    search = {
                    "ISBN": self.Data[i][0],
                    "Autor": self.Data[i][1],
                    "Titulo": self.Data[i][2],
        ...
        if De and A != "0" and Autor == " " and Titulo != " ":
            for i in range(len(self.Data)):
                if int(De)<=self.Data[i][4]<=int(A) and self.Data[i][2] == Titulo:
                    search = {
                    "ISBN": self.Data[i][0],
                    "Autor": self.Data[i][1],
                    "Titulo": self.Data[i][2],
        ...
        if De and A == "0" and Autor != " " and Titulo != " ":
            for i in range(len(self.Data)):
                if  self.Data[i][1] == Autor and self.Data[i][2] == Titulo:
                    search = {
                    "ISBN": self.Data[i][0],
                    "Autor": self.Data[i][1],
                    "Titulo": self.Data[i][2],
...
class Lender:
    ...
    def Return_Book(self,uuid):
        try:
            ...
            return jsonify({"msg":"Ingrese numeros no caracteres "}), 400
...
@app.route('/')
def Welcome():
    return "<H1 align=\"center\"> Bienvenido a la Biblioteca el Amacener </H1>"
...
def GET_Search_Book_Filter():
    ...
    try:
        ...
        return jsonify({"msg":"Colocar Numeros no caracteres en De y A "}),400        
    return book.Search_Book_Filter(Titulo,De,A,Autor)
...
def PATCH_Return_Book(UUID):
    return lender.Return_Book(UUID)

@app.route('/person/<CUI>', methods = ["GET"] )
def GET_Search_Lender(CUI):
    return lender.Search_Lender(CUI)     
